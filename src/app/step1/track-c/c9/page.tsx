...
export default function C9Page() {
  ...
  function handleLetterTap(letter: string, index: number, onComplete: () => void) {
    if (correct) return
  ...
  // Available letters (not yet selected — track by position)
          ...
          <div className="flex justify-center gap-3">
            {letterOrder.map((letter, i) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() => !used && !correct && handleLetterTap(letter, i, onComplete)}
