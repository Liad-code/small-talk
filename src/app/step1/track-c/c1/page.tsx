...
export default function C1Page() {
  ...
  const [tiles, setTiles] = useState(() => shuffle(CVC_WORDS.map(w => ({ id: w.word, word: w.word, vowel: w.vowel, emoji: w.emoji, placed: false }))))
  ...
  const allDone = freeTiles.length === 0
              ...
              {VOWELS.map(v => {
                ...
                return (
                  <div key={v} className="flex flex-col gap-2">
                    {/* Vowel label */}
                    <div className={`
                      w-full py-2 rounded-xl border-4 ${vc.border} ${vc.bg}
                      text-center font-display font-black text-2xl ${vc.text}
                    `}>
                      {v}
                    </div>
                    {/* Drop zone */}
                    <div
                      ...
                      data-expected-ids="[]"
                      data-target-id={v}
                      className={`min-h-[80px] rounded-xl border-2 border-dashed ${vc.border} ${vc.bg} opacity-60 p-1 flex flex-col gap-1 items-center`}
                    >
                      {columns[v].map(wordId => {
                        ...
                        return (
                          <div
                            ...
                            className={`
                              w-full rounded-lg border-2 ${vc.border} ${vc.bg}
            ...
            {freeTiles.length > 0 && (
              <div className="border-t-2 border-dashed border-gray-200 pt-3">
                <p className="text-center text-gray-400 font-bold text-xs mb-3" dir="rtl">
                  לחץ על מילה כדי לשמוע — גרור לעמודה הנכונה:
                </p>
                  ...
                  {freeTiles.map(tile => (
                    ...
                    >
                      <button
                        ...
                        className="text-2xl hover:scale-110 active:scale-90 transition-transform"
                      >{tile.emoji}</button>
                      <DraggableTile
                        id={tile.id}
                        label={tile.word}
