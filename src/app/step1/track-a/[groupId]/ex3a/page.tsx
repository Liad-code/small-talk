...
export default function Ex3aPage({ params }: { params: { groupId: string } }) {
  ...
  // Uppercase list (fixed order), lowercase list (shuffled)
  const [lowerTiles, setLowerTiles] = useState<{ id: string; letter: string; placed: boolean }[]>([])
  // Matched: uppercase → lowercase placed
  ...
  const handleDrop = useCallback((tileId: string, targetEl: Element): boolean => {
    ...
    // targetId is the uppercase letter (stored as lowercase)
                  ...
                  <div
                    ...
                    className={`
                      ...
                      ${matched[letter]
                        ...
                        : 'bg-white border-dashed border-gray-300 drop-hover-ready'}
                    `}
                  >
                    {matched[letter] ? (
